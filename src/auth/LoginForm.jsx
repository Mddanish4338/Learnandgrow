    ...
    try {
      ...
      if (userRole) {
        ...
        toast.success("Login successful!", {
          position: "top-right",
          autoClose: 1000, // Close toast after 1 second
        ...
        const roleCollections = ["students", "trainers", "companies"];
        const queries = roleCollections.map((role) => getDoc(doc(db, role, user.uid)));
  
        // Run Firestore queries in parallel
        const results = await Promise.all(queries);
        const userData = results.find((result) => result.exists())?.data();
        const role = roleCollections[results.findIndex((result) => result.exists())];
